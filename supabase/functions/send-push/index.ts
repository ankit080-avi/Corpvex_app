...
async function sendToToken(token: string, title: string, body: string, data: Record<string, string>) {
  ...
  const message = {
    message: {
      ...
      data,
      android: { priority: 'HIGH', notification: { sound: 'default', channel_id: 'corpvex_default' } },
      webpush: {
        fcm_options: { link: 'https://ankit080-avi.github.io/Corpvex_app/' },
        notification: { icon: 'https://ankit080-avi.github.io/Corpvex_app/icon-192.png' },
      },
    },
  };
...
Deno.serve(async (req) => {
  ...
  const results = await Promise.all(
    tokens.map(async (t: any) => {
      try {
        const r = await sendToToken(t.token, notif.title || 'Corpvex', notif.body || '', data);
